...
disqus_identifier: 1
---
Update (January 22nd, 2018): Thanks to [Ben](https://github.com/TheOtherTimDuncan/TheOtherTimDuncan.github.io/issues/1) for the update for the latest version of Windows 10. The Application Compatibility Toolkit is no longer available, so the [Windows Assessment and Deployment Kit](https://developer.microsoft.com/en-us/windows/hardware/windows-assessment-deployment-kit) is now the source for the Compatibility Administrator. When prompted for which features to install, all you need to select is the Application Compatibility Tools.
